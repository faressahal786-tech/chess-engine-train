...
(function () {
  ...
  const C = G.ChessCore, E = G.Engine, T = G.Trainer;
  ...
  const STRENGTHS = { fast: { depth: 2, timeMs: 800 }, balanced: { depth: 3, timeMs: 2200 }, strong: { depth: 4, timeMs: 6000 } };
  ...
  let engine = null, usingWorker = true, gotHello = false;
  let currentBrainObj = null;
  let pendingHelloBrain = null;
  let trainRunning = false, trainPaused = false;

  function readStoredBrain() {
    try {
      const raw = localStorage.getItem("ct.brain.v1");
      if (!raw) return null;
      const obj = JSON.parse(raw);
      if (obj && obj.params && obj.meta) return obj;
    } catch (e) { void e; }
    return null;
  }
  ...
  async function fetchServerBrain() {
    try {
      const r = await fetch("brains/brain.json", { cache: "no-store" });
      if (!r.ok) return null;
  ...
  function sendHello() {
    engine.send({ cmd: "hello", brain: pendingHelloBrain || undefined });
  }

  function createWorkerBridge() {
    let w;
    try { w = new Worker("js/worker.js"); }
    catch (e) { void e; startInline(); return; }
    usingWorker = true;
    const watchdog = setTimeout(() => {
      if (!gotHello) { try { w.terminate(); } catch (e2) { void e2; } startInline(); }
    }, 3500);
    w.onmessage = (e) => {
      if (!gotHello && e.data && e.data.type === "hello") { gotHello = true; clearTimeout(watchdog); }
      route(e.data);
    };
    w.onerror = (err) => {
      if (!gotHello) { clearTimeout(watchdog); try { w.terminate(); } catch (e2) { void e2; } startInline(); }
      else addTrainLog("worker error: " + (err.message || "unknown"));
    };
    engine = { send: (m) => w.postMessage(m) };
    sendHello();
  }

  function startInline() {
    if (engine && !usingWorker) return;
    usingWorker = false;
    const svc = G.ChessService.create({ reply: (m) => setTimeout(() => route(m), 0) });
    engine = { send: (m) => setTimeout(() => svc.handle(m), 0) };
    addTrainLog("background worker unavailable — running in single-threaded inline mode");
    toast("Worker unavailable — running in single-threaded mode");
    sendHello();
  }
  ...
  function route(msg) {
    ...
    switch (msg.type) {
      case "hello":
      case "loaded":
        adoptBrain(msg.brain, msg.type === "loaded");
        break;
      case "brain":
      ...
      case "started":
        ...
        $("stThreads").textContent = msg.threads || 1;
        setTrainButtons();
        addTrainLog(`training started on ${msg.threads || 1} thread${(msg.threads || 1) > 1 ? "s" : ""}`);
        break;
      case "resumed":
        trainPaused = false;
        setTrainButtons();
      ...
      case "stopped": {
        ...
        adoptBrain(msg.brain, true);
        const s = msg.stats || {};
        addTrainLog(`training stopped · gen ${s.gen || 0} · ${s.games || 0} games total`);
      ...
      case "error":
        ...
        break;
      case "thinkResult":
        handleThinkResult(msg);
        break;
    }
  }

  function adoptBrain(obj, save) {
    if (!obj || !obj.params) return;
    currentBrainObj = obj;
    if (save) {
      try { localStorage.setItem("ct.brain.v1", JSON.stringify(obj)); } catch (e) { void e; }
    }
    const meta = obj.meta || {};
  ...
  function scheduleEngine() {
    ...
    setTimeout(() => {
      if (cur !== epoch) return;
      engine.send({
        cmd: "think", id: thinkId++, fen: C.toFen(pos), epoch: cur,
        opts: { maxDepth: opts.depth, timeMs: opts.timeMs, rootNoise: 8, pickMargin: 16 }
      });
    }, 40);
  }

  function handleThinkResult(res) {
    if (res.epoch !== epoch) return;
    if (res.hint) {
      if (res.ok && res.from >= 0) {
        clearHint();
    ...
    if (!thinking) return;
    thinking = false;
    if (!res.ok || gameOverInfo) { updateStatus(); return; }
    let mv = null;
    for (const m of legalNow) {
  ...
  function updateEvalFromThink(res, moverWhite) {
    ...
    const pv = (res.pvSan || []).slice(0, 6).join(" ");
    $("engineInfo").textContent =
      `depth ${res.depth} \u00B7 ${(res.nodes / 1000).toFixed(1)}k nodes \u00B7 ${(res.timeMs / 1000).toFixed(1)}s` + (pv ? ` \u00B7 ${pv}` : "");
  }
  ...
  function updateStatsUI(s) {
    ...
    $("stAvgPlies").textContent = s.games ? Math.round((s.totalPlies || 0) / s.games) : "\u2013";
    $("stLastMatch").textContent = s.lastGames ? `${s.lastChPts}/${s.lastGames}` : "\u2013";
  }
  ...
  function onGenDone(msg) {
    ...
    addTrainLog(
      `gen ${msg.stats.gen} \u00B7 challenger ${msg.chPts}/${msg.games} pts ` +
      `(${msg.chW}W ${msg.chL}L ${msg.draws}D) \u00B7 avg ${msg.avgPlies} plies${star}`
    );
  }
  ...
  function runSelfTest() {
    ...
    const tests = [
      ["perft startpos d1-d3 (20/400/8902)", () => {
        const p = C.createStart();
        return C.perft(p, 1) === 20 && C.perft(p, 2) === 400 && C.perft(p, 3) === 8902;
      }],
      ["perft kiwipete d2 (2039)", () => C.perft(C.setFen(new C.Position(), kiwi), 2) === 2039],
      ["search finds Qxf7#", () => {
        const p = C.setFen(new C.Position(), scholar);
        const r = E.think(p, E.defaultParams(), { maxDepth: 3 });
        return !!r && C.mFrom(r.move) === C.parseSq("h5") && C.mTo(r.move) === C.parseSq("f7");
      }],
      ["SAN castling O-O", () => {
        const p = C.setFen(new C.Position(), "r3k2r/pppppppp/8/8/8/8/PPPPPPPP/R3K2R w KQkq - 0 1");
        const legal = C.legalMoves(p);
        const m = legal.find((mm) => C.mFrom(mm) === 0x04 && C.mTo(mm) === 0x06);
        return !!m && C.san(p, m, legal) === "O-O";
      }],
      ["FEN roundtrip", () => {
        const a = C.setFen(new C.Position(), kiwi);
        return C.toFen(C.setFen(new C.Position(), C.toFen(a))) === C.toFen(a);
      }]
    ];
    let i = 0;
    function step() {
      ...
      out.textContent += (ok ? "[PASS] " : "[FAIL] ") + name + (err ? " \u2014 " + err.message : "") + "\n";
  ...
  function wireEvents() {
    ...
    $("btnHint").addEventListener("click", () => {
      ...
      const cur = epoch, o = strengthOpts();
      engine.send({
        cmd: "think", id: thinkId++, hint: true, fen: C.toFen(pos), epoch: cur,
        opts: { maxDepth: Math.min(o.depth, 3), timeMs: 1500 }
      });
    });
    ...
    $("btnTrainStart").addEventListener("click", () => {
      ...
      engine.send({
        ...
        opts: {
          ...
          gamesPerGen: parseInt($("inpGamesPerGen").value, 10),
          maxPlies: parseInt($("inpMaxPlies").value, 10),
          openPlies: parseInt($("inpOpenPlies").value, 10),
          strength: parseFloat($("rngStrength").value),
          threads: parseInt($("inpThreads").value, 10) || 0
        }
      });
    });
    $("btnTrainPause").addEventListener("click", () => {
      if (!trainRunning) return;
      if (trainPaused) { engine.send({ cmd: "resume" }); trainPaused = false; }
      else { engine.send({ cmd: "pause" }); trainPaused = true; }
      setTrainButtons();
      addTrainLog(trainPaused ? "paused" : "resumed");
    });
    $("btnTrainStop").addEventListener("click", () => engine.send({ cmd: "stop" }));
    $("rngStrength").addEventListener("input", () => { $("lblStrength").textContent = parseFloat($("rngStrength").value) + "\u00D7"; });

    $("btnExport").addEventListener("click", () => {
      if (!currentBrainObj) { toast("No brain loaded"); return; }
      const blob = new Blob([JSON.stringify(currentBrainObj, null, 1)], { type: "application/json" });
      const a = document.createElement("a");
      a.href = URL.createObjectURL(blob);
      a.download = `chess-brain-gen${currentBrainObj.meta.generation || 0}.json`;
      a.click();
    ...
    $("importFile").addEventListener("change", (e) => {
      ...
      reader.onload = () => {
        try {
          T.deserializeBrain(String(reader.result));
          engine.send({ cmd: "load", brain: JSON.parse(String(reader.result)) });
        } catch (err) { toast("Invalid brain file: " + err.message); }
      };
      reader.readAsText(file);
      e.target.value = "";
    });
    $("btnReset").addEventListener("click", () => {
      if (!confirm("Reset the brain to default parameters? Training progress in this browser will be lost.")) return;
      engine.send({
        cmd: "load",
        brain: {
          version: 1,
          meta: { generation: 0, bornAtGame: 0, record: { w: 0, l: 0, d: 0 } },
          params: E.paramsToPlain(E.defaultParams())
        }
      });
    });
  ...
  async function boot() {
    ...
    buildHeatmaps();
    wireEvents();
    pendingHelloBrain = readStoredBrain();
    if (!pendingHelloBrain) pendingHelloBrain = await fetchServerBrain();
    createWorkerBridge();
